...
def make_yaml(pct, ss):
    job_name    = f"gw-oracle-md-ms-{pct}-ss{ss}"
    ...
    split_code  = _split_code(pct, ss)
    oracle_code = _oracle_pred_code(pct, ss)

    gp_eval = f"""\
  echo "=== GP EVAL md{pct} ss={ss} gps={GPS} ==="
  time python src/scripts/joint/spatial/gp_eval.py \\
    --config {cfg_path} \\
    --pred-path {oracle_path} \\
    --gru-run-sig {run_sig} \\
    --gp-run-tag md_sweep_pct{pct}_ss{ss} \\
    --pretrain-steps 300 \\
    --pretrain-lr 0.01 \\
...
def main():
    ...
    jobs = [(pct, ss) for pct in MD_PCTS for ss in SS_LIST]
    print(f"{'[DRY RUN] ' if args.dry_run else ''}Submitting {len(jobs)} oracle md multiseed jobs...")
    submitted, failed = 0, 0
    for pct, ss in jobs:
        job_name = f"gw-oracle-md-ms-{pct}-ss{ss}"
        yaml_str = make_yaml(pct, ss)
