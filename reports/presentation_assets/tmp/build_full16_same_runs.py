...
OUT = Path('/tmp/full16_same_runs_assets')
...
for model, path in RUNS.items():
    ...
    rmse_pw_h = (
        df.groupby(['id', 'horizon'], as_index=False)
...
summary.to_csv(OUT / 'slide_table_same_runs_full16_metrics.csv', index=False)
horizon.to_csv(OUT / 'slide_table_rmse_per_horizon_same_runs_full16.csv', index=False)
