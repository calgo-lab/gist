"""gru_gp_eval.py — Evaluation for the joint GRU + GP model.

Loads a checkpoint from outputs/GRU_GP_JOINT/GRU_GP_JOINT_{run_sig}/ and
produces predictions + metrics for spatial_test wells.

The evaluation mirrors gp_eval.py:
  - GRU runs on spatial_train wells → predictions per (date, horizon)
  - GP_h conditions on those predictions → predicts at spatial_test wells
  - Metrics saved to run_dir/eval/

Usage:
    python src/scripts/joint/gru_gp_eval.py [--run-sig RUN_SIG]
"""
...
def main():
    ...
    if not run_sig:
        # Try to reconstruct from config (mirrors _resolve_joint_run_sig in gru_gp_train.py)
    ...
    # 3-way split (saved alongside the model)
    ...
    # Spatial_train wells → GRU inputs
    ...
    # Build windows for the TEST period (> VAL_CUTOFF) on spatial_train wells
    ...
    # ------------------------------------------------------------------
    # Run GRU inference on test windows, grouped by end-date
    # ------------------------------------------------------------------
    ...
    # Select evaluation dates (monthly subsample, mirroring gp_eval.py)
    ...
    # Build eval_gws lookup: date_str → {well_id → gws}
    ...
    with torch.no_grad():
        for date_key in eval_date_keys:
            ...
            y_pred_norm = model(xp, xf, xs)  # (n_train_wells, out_len) normalized
            ...
            for h_idx in range(out_len):
                ...
                y_pred_h = y_pred_norm[:, h_idx] * stds_t + means_t  # denormalize
    ...
    # ------------------------------------------------------------------
    # Metrics (same structure as gp_eval.py)
    # ------------------------------------------------------------------
