"""gru_dk_eval.py — Evaluation for the joint GRU + Deep Kriging model.

Identical to gru_gp_eval.py but loads dk_models.pt instead of gp_models.pt.

Usage:
    python src/scripts/joint/gru_dk_eval.py [--run-sig RUN_SIG] [--split test]
"""
...
def main():
    ...
    parser.add_argument("--run-sig", default=None)
    ...
    if not run_dir.exists():
        raise FileNotFoundError(f"DK run directory not found: {run_dir}")
    print(f"Loading from {run_dir}")

    gru_ckpt = torch.load(run_dir / "model.pt", map_location="cpu", weights_only=False)
    dk_ckpt = torch.load(run_dir / "dk_models.pt", map_location="cpu", weights_only=False)
    ...
    eval_ids = set(split_df.loc[split_df["spatial_split"] == f"spatial_{args.split}", "id"])
    ...
    in_len = gru_ckpt["past_input_size"] - len(COV_COLS)  # not used directly
    in_len = int(cfg.get("data", {}).get("in_len", 52))
    out_len = gru_ckpt["out_len"]

    x_past_all, x_future_all, _, x_static_all, meta = _build_windows(
    ...
    x_past_test, x_future_test = _scale(x_past_test, x_future_test)
    ...
    model = GRUSeq2Seq(
        ...
        use_revin=gru_ckpt.get("use_revin", False),
    ).to(device)
    model.load_state_dict(gru_ckpt["model"])
    model.eval()

    dk_config = dk_ckpt["dk_config"]
    dk_models = []
    for h in range(out_len):
        dk = DeepKrigingLayer(
            n_spatial_dims=2,
            n_basis=dk_config["n_basis"],
            hidden_sizes=dk_config["hidden_sizes"],
            lambda_reg=dk_config["lambda_reg"],
        ).to(device)
        dk.load_state_dict(dk_ckpt["dk_state_dicts"][h])
        dk.eval()
        dk_models.append(dk)

    # Group test windows by end-date
    ...
    all_dates_ts = pd.Series(pd.to_datetime(all_dates_sorted), index=pd.to_datetime(all_dates_sorted))
    ...
    with torch.no_grad():
        for date_key in eval_date_keys:
            ...
            for h_idx in range(out_len):
                ...
                y_dk, y_std = dk_models[h_idx].forward_with_std(X_train_sp, y_pred_h, X_eval_sp)

                y_dk_np = y_dk.cpu().float().numpy()
    ...
    if not out_rows:
        raise ValueError("No DK predictions generated.")

    out_df = pd.DataFrame(out_rows)
    print(f"Generated {len(out_df)} predictions, skipped {skipped} (date, horizon) pairs.")
    ...
    per_id_nse = [
        {"id": wid, "NSE_over_time": _nse(g["gws_forecast"].to_numpy(), g["gws_true"].to_numpy())}
    ...
    for h, g in out_df.groupby("horizon"):
        ...
        per_h_rows.append({
            "horizon": int(h), "n": len(real),
            "RMSE": _rmse(pv, real), "nRMSE": _nrmse(pv, real),
    ...
    pq.write_table(pa.Table.from_pandas(out_df), eval_dir / "dk_pred.parquet")
    metrics_df = pd.Series(overall).reset_index()
    metrics_df.columns = ["metric", "value"]
    pq.write_table(pa.Table.from_pandas(metrics_df), eval_dir / "dk_metrics.parquet")
    per_h_df.to_csv(eval_dir / "dk_metrics_by_horizon.csv", index=False)
    if not per_id_nse_df.empty:
        per_id_nse_df.to_csv(eval_dir / "dk_metrics_by_id.csv", index=False)
