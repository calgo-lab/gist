...
rbox(ax, W/2, SPLIT_Y, DW, HB,
     "Spatial split:   train  /  test    ·    GRU uses train wells only",
...
ax.annotate("", xy=(LOSS_CX-1.5, LOSS_Y+LOSS_H/2+G),
    xytext=(W/2+DW/2, TEMP_SPLIT_Y),
    arrowprops=dict(arrowstyle="-|>,head_width=0.90,head_length=0.65",
                    color=C_ARROW, lw=2.5, linestyle="dashed",
...
for k, (fc, ec, lab) in enumerate(box_items):
    ...
    ax.add_patch(FancyBboxPatch((LEG_R-SW_W, iy-SW_H/2), SW_W, SW_H,
        boxstyle="round,pad=0.03", facecolor=fc, edgecolor=ec, linewidth=1.4, zorder=7))
...
OUT = str(Path(__file__).resolve().parents[4] / "reports/figures/architectures/decoupled_flowchart_v2.svg")
