...
C_JOINT_B = "#FF6600"                            # bright orange — joint-only ring
...
GP_EXTRA       = 9.5    # extra data-units below y=0 for sections ⑦⑧
S7_Y           = OUT_Y         - 1.30
...
S8_Y           = EVAL_Y        - 0.75   # section ⑧ header
L_GP_Y    = S8_Y      - 0.90  # L_GP box
L_JOINT_Y = L_GP_Y    - 1.20  # joint loss box
...
# TITLE
ax.text(W/2, TITLE_Y,
        "GRU + GP Joint Training — Groundwater Level Forecasting Architecture",
...
rbox(ax, W/2, SPLIT_Y, DW, 0.70,
     "Three-way spatial split:  spatial_train  /  spatial_val  /  spatial_test  (spatial_test held out entirely)",
     C_OP, C_JOINT_B, fs=10, bold=True, lw=7.5)
...
# ── Evaluation box (spatial_val — used during training) ───────────────────────
...
# ── Final predictions arm (spatial_test) ──────────────────────────────────────
CX_FINAL_GP = 28.0; W_FINAL_GP = 7.0
rbox(ax, CX_FINAL_GP, EVAL_Y, W_FINAL_GP, 0.62,
     "final model  ·  final predictions on spatial test set",
     C_DATA, C_JOINT_B, fs=9.5, bold=True, lw=7.5)
# GP posterior right edge → final spatial predictions
arr(ax, CX_GP + GW_GP/2 + G, GP_OUT_Y,
    CX_FINAL_GP - 1.0, EVAL_Y + 0.31 + G,
    c=C_DATA_B, lw=1.4, rad=0.0)
# spatial split → final spatial predictions (dashed)
ax.annotate("", xy=(CX_FINAL_GP - 1.5, EVAL_Y + 0.31 + G),
            xytext=(W/2 + DW/2, SPLIT_Y - 0.35),
            arrowprops=dict(arrowstyle="-|>,head_width=0.60,head_length=0.45",
                            color=C_ARROW, lw=1.0, linestyle="dashed",
...
L_GP_W = GW_GP
rbox(ax, CX_GP, L_GP_Y, L_GP_W, 0.72,
     "L_GP  =  mean_h [ MSE( ŷ_gp_h ,  y_val_true_h ) ]\nspatial accuracy at spatial_val wells",
     C_LOSS, C_JOINT_B, fs=9.5, bold=True, lw=7.5)
arr(ax, CX_GP, EVAL_Y - 0.31 - G, CX_GP, L_GP_Y + 0.36 + G)

JOINT_CX = W/2; JOINT_W = 14.0
rbox(ax, JOINT_CX, L_JOINT_Y, JOINT_W, 0.62,
     "L  =  L_GRU  +  λ · L_GP       λ = 0.034",
     C_LOSS, C_JOINT_B, fs=11.0, bold=True, lw=7.5)
# L_GP → joint loss
arr(ax, CX_GP, L_GP_Y - 0.36 - G,
    JOINT_CX - 2.0, L_JOINT_Y + 0.31 + G, rad=-0.12)
# MSE Loss → joint loss (long diagonal from right column)
arr(ax, W - 4.5, LOSS_Y - 0.45 - G,
    JOINT_CX + 2.5, L_JOINT_Y + 0.31 + G,
    c=C_OP_B, lw=1.4, rad=0.28)
...
for k, (fc, ec, lab) in enumerate(items):
    lw_leg = 7.5 if ec == C_JOINT_B else 1.2
    r = FancyBboxPatch((lx, ly - k*0.62 - 0.22), 0.44, 0.36,
                       boxstyle="round,pad=0.03",
                       facecolor=fc, edgecolor=ec, linewidth=lw_leg, zorder=7)
    ax.add_patch(r)
    ax.text(lx + 0.60, ly - k*0.62, lab, fontsize=8, va="center", zorder=7)

# ─────────────────────────────────────────────────────────────────────────────
OUT = "reports/figures/joint_flowchart_v2.svg"
