...
def main():
    ...
    baseline_rmse  = next(r for r in rows if r["feat"] == "coords_only")["RMSE_pw"]
    rows_sorted = sorted(rows, key=lambda r: r["nRMSE_pw"])

    print(f"### RAND f90 full oracle GP feature ablation")
    print(f"### 10 split seeds (ss42-ss51) × 1 GP seed, 104 holdout wells")
    print(f"### Baseline coords_only: nRMSE_pw = {baseline_nrmse:.3f}, RMSE_pw = {baseline_rmse:.3f} m\n")

    print(f"| Feature config | nRMSE_pw | ±std | RMSE_pw (m) | ±std | Δ nRMSE | Δ RMSE | n |")
    print(f"|---|---|---|---|---|---|---|---|")
    for r in rows_sorted:
        d_nrmse = r["nRMSE_pw"] - baseline_nrmse
        d_rmse  = r["RMSE_pw"]  - baseline_rmse
        sn = "−" if d_nrmse < 0 else "+"
        sr = "−" if d_rmse  < 0 else "+"
        print(
            f"| {r['feat']} "
            f"| {r['nRMSE_pw']:.3f} | {r['nRMSE_std']:.3f} "
            f"| {r['RMSE_pw']:.3f} | {r['RMSE_std']:.3f} "
            f"| {sn}{abs(d_nrmse):.3f} | {sr}{abs(d_rmse):.3f} "
            f"| {r['n_seeds']} |"
        )
