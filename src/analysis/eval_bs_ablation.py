"""
eval_bs_ablation.py — Compute full metrics for batch-size ablation runs.

Outputs a CSV in the same format as the HPO results table.
Run on the cluster after jobs complete:
    python src/analysis/eval_bs_ablation.py
"""
...
OUT = ROOT / 'reports' / 'gru' / 'metrics' / 'bs_ablation_metrics.csv'
...
def compute(run_sig, batch_size, trial):
    ...
    # objective: global pooled RMSE at h16 from metrics.parquet
    objective = np.nan
    ...
    # per-well per-horizon → median across wells per horizon
    rows = []
