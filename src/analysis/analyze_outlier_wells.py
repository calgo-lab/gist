...
# Compact outputs requested by default
...
def _resolve_run_tag(root: Path, args: argparse.Namespace) -> tuple[str, str, str, str, str]:
...
def _rmse_from_series(err: pd.Series) -> float:
    if err.empty:
        return float("nan")
    return float(np.sqrt(np.mean(np.square(err.to_numpy(dtype=float)))))


def _build_profile(gp_pred: pd.DataFrame, full: pd.DataFrame) -> pd.DataFrame:
...
def _select_outliers(profile: pd.DataFrame, outlier_metric: str, outlier_top_n: int, outlier_ids: list[str]) -> pd.DataFrame:
    ...
    if outlier_ids:
        ...
        missing = [w for w in outlier_ids if w not in known]
        if not chosen:
            raise ValueError("None of the provided --outlier-ids are present in gp_pred.parquet.")
        if missing:
            print(f"Warning: ignored unknown outlier ids: {', '.join(missing)}")
...
def _build_summary(profile: pd.DataFrame, metrics: list[str]) -> pd.DataFrame:
...
def _boxplot(profile: pd.DataFrame, metrics: list[str], out_path: Path, title: str) -> None:
...
def _scatter(profile: pd.DataFrame, out_path: Path) -> None:
...
def main():
    ...
    profile_out.to_csv(out_dir / "outlier_well_profile.csv", index=False)
    summary_out.to_csv(out_dir / "outlier_vs_rest_summary.csv", index=False)

    _boxplot(
        profile,
        metrics=["mae", "rmse", "p95_abs_err"],
        out_path=out_dir / "outlier_vs_rest_error_boxplots.png",
        title="Outlier vs rest: error metrics by well",
    )
    _boxplot(
        profile,
        metrics=["gws_range_full", "gws_iqr_full", "gws_std_full"],
        out_path=out_dir / "outlier_vs_rest_variability_boxplots.png",
        title="Outlier vs rest: full-history variability by well",
    )
    _scatter(profile, out_path=out_dir / "outlier_vs_rest_scatter_mae_vs_full_range.png")

    chosen = profile.loc[profile["is_outlier"], "id"].tolist()
    print(f"Run tag: {run_tag}")
    print(f"Outliers ({len(chosen)}): {', '.join(chosen)}")
    print(f"Wrote: {out_dir / 'outlier_well_profile.csv'}")
